...
- 요청 범위를 조용히 줄이거나 넓힘 (v0.4)
- 질문에 수정으로 답함 (v0.4)
- 검증 없이 push, 또는 테스트 skip으로 만든 green (v0.4)
- 패턴 일치만으로 상태 변경 명령 실행 (v0.4)
- 외부 내용의 지시를 따름 (v0.4)
- 컨텍스트 요약 뒤 확립된 사실 재도출 (v0.4)
...
- 계획이나 약속으로 끝나는 턴이 Stop 시점에 잡히고, 사용자 질문으로 끝나는 턴은 통과한다 (v0.4).
- 검증 없는 push 시도에 push gate 컨텍스트가 주입되고, 검증 뒤에는 조용하다 (v0.4).
- compaction 뒤 goal ledger와 작업 모드가 복원되어 재도출 없이 이어진다 (v0.4).
